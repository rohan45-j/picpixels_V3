(function() {
  ...
  const CATEGORIES = [
    { label: 'Basic Content', key: 'basic', items: ['heading','text','quote'] },
    { label: 'Media Content', key: 'media', items: ['image','full_width_image','gallery'] },
    { label: 'Layout Sections', key: 'layout', items: ['image_with_text','divider','callout'] },
    { label: 'Advanced Sections', key: 'advanced', items: ['faq','list','table','step','stats','cta','code'] },
  ];
  ...
  function renderCard(index) {
    ...
    var card = el('div', { class: 'cb-card', 'data-index': index.toString(), draggable: 'true' });
    if (isExpanded) card.classList.add('is-expanded');

    /* ── Header ── */
    var header = el('div', { class: 'cb-header' });
    var dragHandle = el('div', { class: 'cb-drag' });
    dragHandle.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="8" cy="6" r="1.5"/><circle cx="16" cy="6" r="1.5"/><circle cx="8" cy="12" r="1.5"/><circle cx="16" cy="12" r="1.5"/><circle cx="8" cy="18" r="1.5"/><circle cx="16" cy="18" r="1.5"/></svg>';

    var iconEl = el('span', { class: 'cb-icon' });
    iconEl.textContent = meta.icon;

    var labelEl = el('span', { class: 'cb-label' });
    labelEl.textContent = meta.label;

    var orderEl = el('span', { class: 'cb-order' });
    orderEl.textContent = '#' + (index + 1);

    var previewEl = el('span', { class: 'cb-preview' });
    ...
    var upBtn = el('button', { type: 'button', class: 'cb-btn' });
    upBtn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="18 15 12 9 6 15"/></svg>';
    upBtn.title = 'Move up';
    upBtn.addEventListener('click', function(e) { e.stopPropagation(); moveBlock(index, index - 1); });

    var downBtn = el('button', { type: 'button', class: 'cb-btn' });
    downBtn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="6 9 12 15 18 9"/></svg>';
    downBtn.title = 'Move down';
    downBtn.addEventListener('click', function(e) { e.stopPropagation(); moveBlock(index, index + 1); });

    var expandBtn = el('button', { type: 'button', class: 'cb-btn' });
    ...
    var delBtn = el('button', { type: 'button', class: 'cb-btn cb-btn-danger' });
    delBtn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>';
    delBtn.title = 'Delete';
    ...
    /* ── Body (expanded) ── */
    var body = el('div', { class: 'cb-body' + (isExpanded ? '' : ' cb-body-hidden') });
    ...
    header.addEventListener('click', function(e) {
      if (e.target.closest('.cb-actions, .cb-drag')) return;
      toggleExpand(index);
    });
    ...
    card.addEventListener('dragstart', function(e) {
      ...
      card.classList.add('cb-dragging');
      dragCount = 0;
    });

    card.addEventListener('dragend', function() {
      card.classList.remove('cb-dragging');
      document.querySelectorAll('.cb-drop-indicator').forEach(function(el) { el.remove(); });
      dragCount = 0;
    });
    ...
    card.addEventListener('dragenter', function(e) {
      ...
      if (dragCount === 1) {
        document.querySelectorAll('.cb-drop-indicator').forEach(function(el) { el.remove(); });
        var ind = el('div', { class: 'cb-drop-indicator' });
        card.parentNode.insertBefore(ind, card.nextSibling);
        card.style.borderColor = 'var(--cbp-primary)';
      }
    });
  ...
  function fillFormValues(html, block) {
    for (var key in block) {
      ...
      /* Input values: data-field="KEY" value="" */
      var inputRe = new RegExp('(data-field="' + safeKey + '"\\s*)value=""', 'g');
      html = html.replace(inputRe, '$1value="' + escapeHtml(strVal) + '"');
      ...
      var selRe = new RegExp('(<select[^>]*data-field="' + safeKey + '"[^>]*?>.*?)(<option[^>]*?value="' + regexEscape(escapeHtml(strVal)) + '")', 'g');
      html = html.replace(selRe, function(m, beforeSelect, optionTag) {
        return beforeSelect + optionTag.replace('<option', '<option selected');
      });
    }
    return html;
  }
  ...
  function populateDynamicFields(body, block) {
    ...
    if (Array.isArray(block.images) && block.images.length) {
      ...
      if (list) {
        ...
        block.images.forEach(function(img) {
          var item = el('div', { 'data-item': 'image', class: 'cb-gallery-item' });
          /* Preview thumbnail */
          if (img.src) {
            var prevWrap = el('div', { style: 'margin-bottom:4px' });
            var previewImg = el('img', { src: img.src, alt: '', style: 'max-width:120px;max-height:80px;border-radius:6px;border:1px solid var(--cbp-border);object-fit:cover;display:block' });
            prevWrap.appendChild(previewImg);
            item.appendChild(prevWrap);
          }
          var row = el('div', { class: 'cb-field-row' });
          var srcInp = el('input', { type: 'text', 'data-field': 'src', value: img.src || '', class: 'cb-input', placeholder: 'Image URL', style: 'flex:2' });
          var altInp = el('input', { type: 'text', 'data-field': 'alt', value: img.alt || '', class: 'cb-input', placeholder: 'Alt text', style: 'flex:1' });
          var capInp = el('input', { type: 'text', 'data-field': 'caption', value: img.caption || '', class: 'cb-input', placeholder: 'Caption', style: 'flex:1' });
          var delBtn = el('button', { type: 'button', 'data-remove-btn': '', class: 'cb-btn-sm', title: 'Remove' });
          delBtn.textContent = '✕';
    ...
    if (Array.isArray(block.items) && block.items.length) {
      ...
      if (itemsList) {
        ...
        block.items.forEach(function(text) {
          var item = el('div', { 'data-item': 'item', style: 'display:flex;gap:4px;margin-bottom:4px' });
          var inp = el('input', { type: 'text', 'data-field': 'items[]', value: text || '', class: 'cb-input', placeholder: 'Item text', style: 'flex:1' });
          var delBtn = el('button', { type: 'button', 'data-action': 'remove-item', class: 'cb-btn-sm', title: 'Remove' });
          delBtn.textContent = '✕';
    ...
    if (columnsList) {
      ...
      headers.forEach(function(header, colIdx) {
        var colTag = el('span', { class: 'cb-column-tag', style: 'display:inline-flex;align-items:center;gap:4px;padding:4px 8px;background:var(--cbp-bg);border:1px solid var(--cbp-border);border-radius:6px;font-size:0.75rem;font-weight:500' });
        colTag.textContent = header || 'Column ' + (colIdx + 1);
        var removeBtn = el('button', { type: 'button', class: 'cb-btn-sm', style: 'background:none;border:none;color:#ef4444;cursor:pointer;padding:0;line-height:1;font-size:0.7rem', title: 'Remove column', 'data-col-index': String(colIdx) });
    ...
    if (Array.isArray(block.rows) && block.rows.length) {
      ...
      if (rowsList) {
        ...
        block.rows.forEach(function(row) {
          var item = el('div', { 'data-item': 'row', style: 'display:flex;gap:4px;margin-bottom:4px' });
          (row || []).forEach(function(cell) {
            var inp = el('input', { type: 'text', value: cell || '', class: 'cb-input', placeholder: '…', style: 'flex:1' });
            item.appendChild(inp);
          });
          var delBtn = el('button', { type: 'button', 'data-remove-btn': '', class: 'cb-btn-sm', title: 'Remove' });
          delBtn.textContent = '✕';
  ...
  function readBlockFromDOM(index) {
    ...
    if (!card) return;
    var body = card.querySelector('.cb-body');
    if (!body) return;
    var block = blocks[index];
  ...
  function renderPreview(index) {
    var card = containerEl.querySelector('[data-index="' + index + '"]');
    if (!card) return;
    var previewEl = card.querySelector('.cb-preview');
    if (!previewEl) return;
  ...
  function addListItem(body, index) {
    ...
    if (!list) return;
    var item = el('div', { 'data-item': 'item', style: 'display:flex;gap:6px;margin-bottom:4px' });
    var inp = el('input', { type: 'text', 'data-field': 'items[]', placeholder: 'Item text', style: 'flex:1;padding:7px 10px;border:1px solid var(--cbp-border);border-radius:6px;font-size:0.82rem;font-family:inherit' });
    var delBtn = el('button', { type: 'button', 'data-action': 'remove-item', style: 'background:none;border:1px solid var(--cbp-border);border-radius:6px;cursor:pointer;font-size:0.75rem;color:#ef4444;padding:4px 10px;transition:all 0.12s' });
    delBtn.textContent = '✕';
    item.appendChild(inp);
    item.appendChild(delBtn);
    list.appendChild(item);
  ...
  function addGalleryImage(body, index) {
    ...
    var item = el('div', { 'data-item': 'image', class: 'cb-gallery-item' });
    var row = el('div', { class: 'cb-field-row' });
    var srcInp = el('input', { type: 'text', 'data-field': 'src', value: '', class: 'cb-input', placeholder: 'Image URL', style: 'flex:2' });
    var altInp = el('input', { type: 'text', 'data-field': 'alt', value: '', class: 'cb-input', placeholder: 'Alt text', style: 'flex:1' });
    var capInp = el('input', { type: 'text', 'data-field': 'caption', value: '', class: 'cb-input', placeholder: 'Caption', style: 'flex:1' });
    var delBtn = el('button', { type: 'button', 'data-remove-btn': '', class: 'cb-btn-sm', title: 'Remove' });
    delBtn.textContent = '\u2715';
    row.appendChild(srcInp);
    row.appendChild(altInp);
  ...
  function addTableRow(body, index) {
    ...
    var row = el('div', { 'data-item': 'row', style: 'display:flex;gap:4px;margin-bottom:4px' });
    for (var i = 0; i < cols; i++) {
      var inp = el('input', { type: 'text', placeholder: '…', style: 'flex:1;padding:6px 8px;border:1px solid var(--cbp-border);border-radius:6px;font-size:0.82rem;font-family:inherit' });
      inp.addEventListener('input', function() { onFieldChange(index); });
      row.appendChild(inp);
    }
    var delBtn = el('button', { type: 'button', style: 'background:none;border:1px solid var(--cbp-border);border-radius:6px;cursor:pointer;font-size:0.75rem;color:#ef4444;padding:4px 8px;transition:all 0.12s' });
    delBtn.textContent = '\u2715';
  ...
  function addTableColumn(body, index) {
    ...
    if (headerField) {
      ...
      headerField = el('input', { type: 'text', 'data-field': 'headers', value: newHeader, class: 'cb-input' });
    }
    ...
    list.querySelectorAll('[data-item="row"]').forEach(function(rowEl) {
      ...
      if (delBtn) {
        rowEl.insertBefore(inp, delBtn);
      } else {
        rowEl.appendChild(inp);
      }
    });

    /* If no rows exist, create one with the new column */
    if (list.querySelectorAll('[data-item="row"]').length === 0) {
      addTableRow(body, index);
    }

    /* Refresh columns manager */
  ...
  function removeColumnAt(body, index, colIdx) {
    ...
    var headers = headerField.value ? headerField.value.split(',').map(function(s) { return s.trim(); }).filter(function(s) { return s; }) : [];
    if (headers.length <= 1) {
      /* Don't allow removing the last column via this method */
      return;
    }
    ...
    list.querySelectorAll('[data-item="row"]').forEach(function(rowEl) {
      ...
      if (inputs.length > colIdx) {
        inputs[colIdx].remove();
      }
    });
  ...
  /* ── Proper reorder: remove from old position, insert at target ── */
  ...
  /* ── Modal: use 'flex' to match CSS instead of 'block' ── */
  function openPicker() {
