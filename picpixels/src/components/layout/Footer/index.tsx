...
const FALLBACK_SERVICES: Service[] = [
  ...
  { id: 2, title: 'Background Removal', slug: 'background-removal-service' } as Service,
  { id: 3, title: 'Image Masking', slug: 'image-masking-service' } as Service,
  { id: 4, title: 'Ghost Mannequin', slug: 'ghost-mannequin-service' } as Service,
  { id: 5, title: 'Photo Retouching', slug: 'photo-retouching-service' } as Service,
];
...
export default function Footer({ siteSettings: serverSettings, footerServices: serverFooterServices }: { siteSettings?: SiteSetting | null; footerServices?: Service[] }) {
  ...
  useEffect(() => {
    if (!serverFooterServices) {
      fetchFooterServices().then(svcs => { if (svcs.length > 0) setFooterServices(svcs); });
    }
        ...
        <div className={styles.grid}>
          <div className={styles.brandCol}>
            <HomeLink className={styles.logo}>
              {siteSettings?.logo && mediaUrl(siteSettings.logo) ? (
                <OptimizedImage src={mediaUrl(siteSettings.logo)!} alt={siteSettings.logo_alt || siteSettings.site_name || 'PicPicxels'} width={180} height={50} />
              ) : (
          ...
          <div className={styles.linksCol}>
            ...
            <ul className={styles.list}>
              ...
              <li>
                <Link href="/services" className={styles.allLink}>View All Services →</Link>
              </li>
            </ul>
          </div>
